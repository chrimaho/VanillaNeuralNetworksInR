...
```{r Check Data, eval=TRUE, echo=FALSE, results="asis", message=FALSE, warning=FALSE}
...
let_TrainModel <- function(x_train, y_train,
    ...
    for (epoch in 1:epochs) {
        ...
        network_model <- set_ApplyDifferentiateCost(
            network_model=network_model, 
            get_DifferentiateCost(network_model[["output"]][["acti"]], y_train)
        )
    ...
    if (print_learning_curve == TRUE) {
        ...
        print(output[["results"]][["cost"]] %>% 
            ...
            labs(
                ...
                subtitle=paste0(
                    "Inp: '", input_nodes, "',  Hid: '", paste0(hidden_nodes, collapse=","), "',  Out: '", output_nodes, "'\n",
...
network_model <- training_output[["network_model"]]
training_cost <- training_output[["results"]][["cost"]]
...
training_output <- let_TrainModel(
    x_train=trn_img, y_train=trn_cls,
    input_nodes=dim(trn_img)[2], hidden_nodes=c(1000,500,100,50,1), output_nodes=1,
    initialisation_algorithm="he", initialisation_order=2,
    epochs=500, learning_rate=0.001,
    activation_hidden="relu", activation_final="sigmoid",
    verbosity=50
...
library(ROCR)
pred <- get_Prediction(tst_img, tst_cls, network_model)
pred <- prediction(pred$probs, pred$truth)
rocr <- performance(pred, measure="tpr", x.measure="fpr")
plot(rocr)

library(plotROC)
get_Prediction(tst_img, tst_cls, network_model) %>% 
    ggplot(aes(m=probs, d=truth)) +
    geom_roc(n.cuts=0) +
    style_roc(theme=theme_grey)

get_Prediction(tst_img, tst_cls, network_model) %>% select(truth, class) %>% distinct()
    caret::confusionMatrix(data=.["class"], reference=.["truth"])
```
...
```{r}
pred <- get_Prediction(tst_img, tst_cls, network_model, 0.16) 
...
pred %>% select(truth, class) %>% distinct()
...
pred %<>% 
    mutate(class=ifelse(class=="Yes","Car","Other"),
           truth=ifelse(truth=="No","Other","Car")) %>% 
    mutate_at(c("class","truth"), factor)

table(pred$truth)
table(pred$class)
caret::confusionMatrix(pred$class, pred$truth, positive="Car")
conf <- caret::confusionMatrix(pred$class, pred$truth, positive="Car")
```
