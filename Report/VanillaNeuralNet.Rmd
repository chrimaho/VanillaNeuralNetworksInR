...
date: 'Special thanks to: [Alex Scriven](https://www.linkedin.com/in/alexjscriven/)'
toc-title: Contents
output:
    html_notebook: 
        code_folding: none
...
What this article _actually_ is, is a functional walk-through, how-to piece, for creating a Vanilla Neural Network (a Feed-Forward Network), from scratch, step-by-step, in the `R` programming language. It contains lots of code, and lots of technical details.
...
1. All images are the same the same size ($32$x$32$ pixels);
...
```{r Get Data, eval=TRUE, echo=TRUE, messages=FALSE, errors=FALSE, warning=FALSE}
# Download Data
# NOTE:
...
One of the challenges behind accessing this data from the `tensorflow` package is that the classes are only numeric values (`0` to `9`) for each type of image. The definitions for these images can be found on GitHub ([GitHub > EN10 > CIFAR](https://github.com/EN10/CIFAR#classes)). These classes are defined in the following code chunk.
...
In order to visualise the images, two custom functions are written, as shown in the following code chunk. These functions take in the data (as a 4-Dimensional array), and visualise the images as a plot. Note, the source code for the custom function `set_MakeImage()` can be found [here](https://github.com/chrimaho/VanillaNeuralNetworksInR/blob/master/Functions/functions.R).
...
When the function is run on the top 16 images, the following is displayed. As shown, the images are extremely pixelated (which is expected, as they are only `32`x`32` pixels each), and you can see how each of the images are categorised.

```{r View Images, eval=TRUE, echo=TRUE, fig.width=12, fig.height=12, message=FALSE}
# Set list
lst <- list()
...
```{r Save View Images, eval=TRUE, echo=FALSE, message=FALSE}
# Save output
ggsave("./Images/ViewImages.jpg", plt)
```
...
cifar[["classes"]] <- cifar %>%
    ...
    (function(classes){
        ...
        classes %>% as.vector %>% head(30) %>% print()
...
```{r Plot Original Classes, eval=TRUE, echo=TRUE, message=FALSE}
# Print Plot
cifar %>% 
    extract("classes") %>% 
    table(dnn="classes") %>% 
...
```{r Save Plot Original Classes, eval=TRUE, echo=FALSE, message=FALSE}
# Save Plot
ggsave("./Images/PlotOriginalClasses.jpg")
```
...
```{r Plot Split Classes, eval=TRUE, echo=TRUE, message=FALSE}
# Print Plot
rbind(
    trn_cls %>% table,
    tst_cls %>% table
...
```{r Save Plot Split Classes, eval=TRUE, echo=FALSE, message=FALSE}
# Save Plot
ggsave("./Images/PlotSplitClasses.jpg")
```
...
## Define the Architecture {#DefineArchitecture}
...
## Set the Instantiation Function {#SetInstantiationFunction}
...
```{r Model Function: Set Instantiate Network, eval=FALSE, echo=TRUE}
...
## Create the Network {#CreateNetwork}
...
## Visualise the Network {#VisualiseNetwork}
...
Further visualisation can be conducted once the network is fully initialised and forward propagated. See section [Check Model Shapes](#CheckShapes) for more details.
...
## Weight Initialisation {#WeightInitialisation}
...
### Xavier Algorithm {#XavierAlgorithm}
...
* $n_i$ is the number of nodes coming in to this layer. Aka "fan-in".

## Initialisation Functions {#InitialisationFunctions}
...
```{r Model Function: Let Initialise Xavier, eval=FALSE, echo=TRUE}
...
```{r Model Function: Let Initialise He, eval=FALSE, echo=TRUE}
...
## Layer Initialisation {#LayerInitialisation}
...
```{r Model Function: Set Initialise Layer, eval=FALSE, echo=TRUE}
...
```{r Model Function: Set Initialise Model, eval=FALSE, echo=TRUE, warning=FALSE}
...
## Network Initialisation {#NetworkInitialisation}
...
## Check the Model Parameters {#CheckModelParameters}
...
```{r Check Model Parameters, eval=TRUE, echo=TRUE}
network_model %>% 
...
# Forward Propogation {#ForwardPropagation}

## The Theory {#TheTheory}

There is a very good website which outlines what actually happens during a matrix multiplication: [Matrix Multiplication](http://matrixmultiplication.xyz/).
...
$$
[Matrix_{input}] \times [Matrix_{weight}] = [Matrix_{layer}] \\
\\ 
\begin{bmatrix}
 1 &  2 &  3 \\
 4 &  5 &  6 \\
...
```{r Run Step One of Matrix Algebra, eval=TRUE, echo=TRUE}
...
```
```{r Print Step One of Matrix Algebra, eval=TRUE, echo=FALSE}
writeLines("matrix_input:")
matrix_input
writeLines("\nmatrix_weight:")
matrix_weight
writeLines("\nmatrix_layer:")
...
$$
[Matrix_{layer}] + [Vector_{bias}] = [Matrix_{biased}] \\ 
\\ 
\begin{bmatrix}
 46 &  52 &  58 &  64 &  70 \\
100 & 115 & 130 & 145 & 160 \\
...
```{r Run Step Two of Matrix Algebra, eval=TRUE, echo=TRUE}
# Declare Bias matrix
...
```
```{r Print Step Two of Matrix Algebra, eval=TRUE, echo=FALSE}
writeLines("matrix_layer:")
matrix_layer
writeLines("\nvector_bias:")
vector_bias
writeLines("\nmatrix_biased:")
...
$$
[Matrix_{biased}] \times Activation = [Matrix_{output}] \\ 
\\ 
\begin{bmatrix}
 47 &  53 &  59 &  65 &  71 \\
102 & 117 & 132 & 147 & 162 \\
...
```{r Run Step Three of Matrix Algebra, eval=TRUE, echo=TRUE}
# Apply Activation function
matrix_output <- matrix_biased * (0.01 * matrix_biased)
```
```{r Print Step Three of Matrix Algebra, eval=TRUE, echo=FALSE}
writeLines("matrix_biased:")
matrix_biased
writeLines("\nmatrix_output:")
matrix_output
```
...
```{r Model Function: Set Linear Forward, eval=FALSE, echo=TRUE}
...
## Non-Linear Component {#NonLinearComponent}
...
```{r Model Function: Set Activation Sigmoid, eval=FALSE, echo=TRUE}
...
```
```{r Model Function: Set Activation Relu, eval=FALSE, echo=TRUE}
...
```
```{r Model Function: Set Activation Softmax, eval=FALSE, echo=TRUE}
...
```
```{r Model Function: Set Activation Swish, eval=FALSE, echo=TRUE}
...
## Set Forward Propagation Function {#SetForwardPropagationFunction}
...
```{r Model Function: Set Foward Propagation, eval=FALSE, echo=TRUE}
...
## Run Forward Propagation {#RunForwardPropagation}
...
```{r Run Forward Propagation, eval=TRUE, echo=TRUE}
tic()
network_model %<>% set_ForwardProp(trn_img, "relu", "sigmoid")
toc()
```

As mentioned above in the [Split Data](#SplitData) section, the `trn_img` object is over `82 Mb` large. The entire end-to-end process for forward propagation took only `7 seconds` to run; which is quite impressive, and is a testament to the power of mathematics.

## Check Model Shapes {#CheckShapes}

The Network can now be printed using the custom function `get_PrintNetwork()`. This function returns each layer in an individual box, containing key information like the relevant shapes of the matrices and the activation functions used. The source code for the function can be found [here](https://github.com/chrimaho/VanillaNeuralNetworksInR/blob/master/Functions/functions.R).

```{r Visualise The Network, eval=TRUE, echo=TRUE}
# Print the Network
network_model %>% 
    get_PrintNetwork() %>% 
    cat()
...
# Calculate the Cost {#CalculateCost}
...
## Set Cost Function {#SetUpCostCalculation}
...
```{r Model Function: Get Compute Cost, eval=FALSE, echo=TRUE}
...
```{r Model Function: Set Apply Cost, eval=FALSE, echo=TRUE}
...
## Differentiate Cost {#DifferentiateCost}
...
```{r Model Function: Get Differentiate Cost, eval=FALSE, echo=TRUE}
...
```{r Model Function: Set Apply Differentiate Cost, eval=FALSE, echo=TRUE}
...
```{r Run and Apply Cost Differential, eval=TRUE, echo=TRUE}
network_model %<>% set_ApplyDifferentiateCost(
    get_DifferentiateCost(network_model[["output"]][["acti"]],trn_cls)
)
```

## Differentiate Activation {#DifferentiateActivations}
...
```{r Model Function: Let Backward Activate Relu, eval=FALSE, echo=TRUE}
...
```
```{r Model Function: Let Backward Activate Sigmoid, eval=FALSE, echo=TRUE}
...
## Differentiate Linear {#DifferentiateLinear}
...
Notice here that the output is actually a list of three elements. This is unique to `R`, as this language can only ever output a single element for functions; as compared to `python` which is able to return multiple elements from each function.

```{r Model Function: Get Differentiate Linear, eval=FALSE, echo=TRUE}
...
## Run Backward Propogation {#RunBackwardPropagation}
...
### Set Backward Propagation Function {#SetBackwardPropagationFunction}

The backward propagation function to be defined (`set_BackwardProp()`) must be designed to run through the following steps:

1. Run through each layer in _reverse_ direction. This is necessary because, quite logically, the backward propagation function needs to be run in a backwards direction.
1. Extract the layer name.
1. Skip the `input` layer. Again, quite logical, because this layer is at the beginning of the network, and does not need to be back-propagated.
1. Extract the name of the previous layer.
1. Extract the relevant matrices to be used for subsequent calculations.
1. Extract the relevant activation function for this particular layer.
1. Set up some empty matrices, which will house the relevant differentiated matrices.
1. Differentiate the `activation` matrix of the _current_ layer.
1. Differentiate the `linear` matrices, including:

    1. `weight` matrix of the _current_ layer.
    1. `bias` matrix of the _current_ layer.
    1. `activation` matrix of the _previous_ layer.
    
1. Apply information back in to the relevant places in the `network_model`.
1. Return the updated `network_model`.

```{r Model function: Set Backward Propagation, eval=FALSE, echo=TRUE}
...
### Run Backward Propagation Function {#RunBackwardPropagationFunction}

Having defined this function, the next step is to run the function. The below code chunk is wrapped with the `tic()` and `toc()` functions, in order to determine how mucht time it took for the function to run.

```{r Run Backward Propagation, eval=TRUE, echo=TRUE}
tic()
network_model %<>% set_BackwardProp()
toc()
```

As shown, it took approximately `8 seconds` to run this function. This is quite impressive, considering that there are over 320,000 parameters to be updated (see [Check Model Parameters](#CheckModelParameters) section).

# Update Model Parameters {#UpdateModelParameters}
...
The method used to update these parameters is called [Stochastic Gradient Descent](https://en.wikipedia.org/wiki/Stochastic_gradient_descent). For more information, see [Stochastic Gradient Learning in Neural Networks](https://leon.bottou.org/publications/pdf/nimes-1991.pdf) or [Backpropagation and Stochastic Gradient Descent Method](https://www.sciencedirect.com/science/article/abs/pii/092523129390006O).

For sure, there are other methods of implementing neural network optimisation. The literature has been spending a lot of effort in this area. There are algorithms such as [RMSProp](https://towardsdatascience.com/understanding-rmsprop-faster-neural-network-learning-62e116fcf29a) and [Adam](https://towardsdatascience.com/adam-latest-trends-in-deep-learning-optimization-6be9a291375c), which implement intelligent methods to achieve a faster convergence, and a more accurate final result. Some good sources for this include [An Empirical Comparison of Optimizers for Machine Learning Models](https://heartbeat.fritz.ai/an-empirical-comparison-of-optimizers-for-machine-learning-models-b86f29957050) and [Overview of Different Optimizers for Neural Networks](https://medium.com/datadriveninvestor/overview-of-different-optimizers-for-neural-networks-e0ed119440c3). For further enhancement and optimisation, it is important to explore these optimisation options.

Nonetheless, the process of implementing Stochastic Gradient Descent is really quite simple:

1. Specify a given _learning rate_ between `0` and `1` (usually very very small, for example `0.001`).
1. Take the differenced _weight_ and _bias_ matrices, and multiply by the _learning rate_ in a **negative** direction.
1. Add the updated differenced _weight_ and _bias_ matrices, and add to the original _weight_ and _bias_ matrix.
1. Repeat this process layer-by-layer.

## Set the Update Model Function {#SetUpdateModelFunction}
...
1. Loop through each layer in the network in a _forward_ direction.
1. Extract the layer name.
1. Skip the `input` layer (as it does not need updating).
1. Define the gradient steps for the `back_wgts` and `back_bias` matrices.
1. Apply the gradient steps to the original `wgts` a `bias` matrices.
1. Return the updated mode.

```{r Model Function: Set Update Model, eval=FALSE, echo=TRUE}
...
## Run the Update Model Function {#RunUpdateModelFunction}

Having defined the function, then we run the model our network.

```{r Run Update Model, eval=TRUE, echo=TRUE}
network_model %<>% set_UpdateModel(0.01)
```

# Run the Model End-to-End {#RunModelEndToEnd}
...
Each time this is repeated is called an _Epoch_. It is quite typical for networks to be updated over many many Epochs. Sometimes hundreds, sometimes thousands of Epochs; the exact number of Epochs to run is a discretionary decision by the Data Scientist, based on numerous variables.

There is one additional step to be added in here, and that is _batching_ the data. For this, we can consider within each Epoch, the data will be batched up in to equally divisible groups, and used for subsequent processing. In this instance, the model parameters will be updated after each batch. And when the full amount of data has been run through the model in it's entirety, this is then considered one Epoch.

## Set up the Train Model Function {#SetTrainModel}

To illustrate this programmitically, the following function is written.

Notice here, that there are many custom functions included (the source code for each of which can be found [here](https://github.com/chrimaho/VanillaNeuralNetworksInR/blob/master/Functions/functions.R)). These functions include:
...
1. Begin the timer.
1. Declare what information will be returned.
1. Instantiate the network.
1. Initialise the network.
1. Loop through each Epoch.
1. Loop through each batch.
1. Subset the data for this particular batch.
1. Run forward propagation.
1. Calculate the cost.
1. Apply the cost to the network.
1. Differentiate the cost.
1. Run backward propagation. 
1. Update the model parameters.
1. Run through next batch. 
1. Save the overall cost at the end of the Epoch.
1. Print the update at the relevant Epoch number (from the `verbosity` argument)
1. Run through next Epoch.
1. Save the updated network.
1. Print the learning curve.
1. Return the output.

```{r Model Function: Let Train Model, eval=FALSE, echo=TRUE}
let_TrainModel <- function(
    ...
    for (epoch in 1:epochs) {
        ...
        for (batch in 1:batches) {
            ...
            network_model <- set_ApplyDifferentiateCost(
                network_model = network_model, 
                cost_differential = get_DifferentiateCost(
...
## Run the Train Model Function {#RunTrainModel}

Having had set up the training function, let' now run it.

Notice here that:

1. Only the training data is used (see [Split Data](#SplitData) section).
1. The `He` initialisation algorithm is used, with an order of `2`.
1. The `relu` activation algorithm is used for the hidden layers, with the `sigmoid` activation algorithm for the output layer.
1. There are `56` batches, and `50` epochs.
1. The model results are printed every `10` epochs.
1. The learning curve is printed at the end.

```{r Run Model Training 1, eval=TRUE, echo=TRUE, message=FALSE}
...
```{r Save Model Training 1, eval=TRUE, echo=FALSE, message=FALSE}
# Save output
ggsave("./Images/ModelTraining1.jpg")
```


It works! This plot proves that the model is training, and it is continuing to learn and improve its performance over time.

Notice here that the cost line begins at approximately `0.4`, and very quickly decreases to `0.3` after 20 Epochs. In order for it to complete the full `50` Epochs, this took approximately `11 minutes`. This can be seen as a success.

## Further Experimentation {#FurtherExperimentation}
...
1. A different initialisation algorithm, or initialisation order (change the `initialisation_alghorithm` or `initialisation_order` parameter),
1. A different activation function on the hidden layers (change the `activation_hidden` parameter),
1. A different number of batches per Epoch (change the `batches` parameter),
1. A different number of epochs (change the `epochs` parameter),
1. A different learning rate (change the `learning_rate` parameter),
1. Get more data (recall in the [Download Data](#DownloadData) section, only $10,000$ images were downloaded; but there are another $50,000$ images which can be used for further training).

Here is an experimentation of trying some different parameters. Notice how the results also change. This next round of training took `33 minutes` to run.

```{r Run Model Training 2, eval=TRUE, echo=TRUE, message=FALSE}
training_output <- let_TrainModel(
    x_train=trn_img, 
    y_train=trn_cls,
    input_nodes=dim(trn_img)[2],
...
```
```{r Save Model Training 2, eval=TRUE, echo=FALSE, message=FALSE}
# Save output
ggsave("./Images/ModelTraining2.jpg")
```


It is then the job of the Data Scientist to identify the optimal configuration of these parameters, in order to achieve a better performance rate. See [Hyperparameter Optimization in Machine Learning Models](https://www.datacamp.com/community/tutorials/parameter-optimization-machine-learning-models) for more details.

However, as this article is not to _achieve_ the best results, but more to _show how_ to achieve said results, and the processes/methodologies involved, then further experimentation will not be conducted.
...
## Set Up Prediction {#SetUpPrediction}

The first step is to write two custom functions. The first will take in the test data and trained model, and will return a dataframe containing the predicted values and the truth values. The pretty-print the Confusion Matrix in a `ggplot`-style of output.

```{r Model Function: Get Prediction, eval=FALSE, echo=TRUE}
...
```{r Model Function: Plot Confusion Matrix, eval=FALSE, echo=TRUE}
...
The next step is to actually run the prediction. This step is quite self explanatory. The parameters for this function includes the test data, and the trained mode.

```{r Run Prediction, eval=TRUE, echo=TRUE}
# Create prediction
...
## View Prediction {#ViewPrediction}

Having created this prediction, it's then necessary to visualise the output. In the same way as the [Check Images](#CheckImages) section, the following code chunk visualises the top 16 images, and returns a label for each.

As seen, the model is clearly able to recognise some, but it also makes mistakes on others.

```{r View Prediction, eval=TRUE, echo=TRUE, fig.width=12, fig.height=12}
# Define classes
...
```{r Save View Prediction, eval=TRUE, echo=FALSE, message=FALSE}
ggsave("./Images/ViewPrediction.jpg", plt)
```


## Test the Prediction {#TestPrediction}
...
Since this article is about _showing_ the method, then we will not progress any further with more optimisation here.

```{r Create Confusion Matrix, eval=TRUE, echo=TRUE}
# Set Up
...
Next step is to plot the confusion matrix. A good source of understanding and interpreting confusion matrices can be found here: [Confusion Matrix](https://en.wikipedia.org/wiki/Confusion_matrix). In this website, it also includes an image (also copied below) for a very good visual understanding.

![*Confusion Matrix Interpretation*](./Images/Confusion Matrix.png)

When we visualise the confusion matrix of our model, we can see that successfully predicted a majority of the images. However, the largest number of bad classifications is where it predicted that the image was a car, when it actually was not.

```{r Plot Confusion Matrix, eval=TRUE, echo=TRUE, fig.width=5, fig.height=5, message=FALSE}
# Plot Confusion Matrix
ConfusionMatrix %>% 
    plt_ConfusionMatrix()
```

```{r Save Confusion Matrix, eval=TRUE, echo=FALSE, message=FALSE}
ggsave("./Images/ConfusionMatrix.jpg")
```

Another good analytical and plotting tool is to use the ROC Curve (receiver operating characteristics). An ROC Curve is a graphical plot that illustrates the diagnostic ability of a binary classifier system as its discrimination threshold is varied (see [Receiver Operating Characteristic](https://en.wikipedia.org/wiki/Receiver_operating_characteristic)).

Essentially, a model that creates a perfect prediction would have the curve perfectly 'hugging' the top-left of this plot. As our model is not doing this, it is clearly obvious that further training and optimisation is necessary. For this, it is necessary to continue experimenting as mentioned in the [Further Experimentation](#FurtherExperimentation) section.

```{r Plot ROC, eval=TRUE, echo=TRUE, fig.width=5, fig.height=5, message=FALSE}
# Print ROC
Prediction %>% 
    ggplot() +
...
```{r Save ROC Curve, eval=TRUE, echo=FALSE, message=FALSE}
ggsave("./Images/ROCCurve.jpg")
```
...
It has also been shown how to do this entirely in `R`, and without using any pre-defined deep learning framework. For sure, there are other packages that can perform all of the steps here, and perhaps in a more computationally efficient manner. But the use of these packages is not the target of this article. Here, the aim is to dispel the 'blackbox' phenomenon, and to show how to create these deep learning frameworks from the ground up.

As shown, the architecture of these feed-forward neural networks effectively use [matrix multiplication](https://en.wikipedia.org/wiki/Matrix_multiplication) and [differential calculus](https://en.wikipedia.org/wiki/Differential_calculus) in order to adjust the 'weights' of a network, and increase it's predictive power. It does require lots of data, and does require a long time to train and optimise. Furthermore, there are many (many, _many_) different architectural choices that can be made along the way, all of which are at the discretion of the Data Scientist applying this methodology.

At the end of the day, this article has proven that deep learning can definitely be achieved in `R`. Thereby, dispelling the myth that 'in order to do Deep Learning, you must use `Python`'. Certainly, there are many successful implementations of Deep Learning in `Python`; for example: [Building a Feedforward Neural Network from Scratch in Python](https://hackernoon.com/building-a-feedforward-neural-network-from-scratch-in-python-d3526457156b). There's also example of achieving the same in Swift ([Deep Neural Networks in Swift, lessons learned](https://www.tooploox.com/blog/deep-neural-networks-in-swift-lessons-learned)), and in C++ ([Neural Network Implementation in C++ From Scratch](https://www.geeksforgeeks.org/ml-neural-network-implementation-in-c-from-scratch/)), and in Java ([Implementing an Artificial Neural Network in Pure Java (No external dependencies)](https://medium.com/coinmonks/implementing-an-artificial-neural-network-in-pure-java-no-external-dependencies-975749a38114)). Therefore, the specific language used is effectively irrelevant. What matters is the use case, the environment, the business artifacts, and the language that the Data Scientist feels comfortable in.
