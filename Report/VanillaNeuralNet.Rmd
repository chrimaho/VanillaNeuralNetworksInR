...
It is important to check the data, to ensure that it has been generated correctly, and all the information looks okay. For this, a custom-function is written (`get_ObjectAttributes`), the source code for which can be found [here]().

```{r Check Data, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
# Print result
cifar %>% 
    extract("images") %>% 
    get_ObjectAttributes() %>% 
...
for (i in 1:20) {
    plt_PlotImage(cifar$images, cifar$classes, i)
}
```
...
```{r}
cifar %>% extract2("classes") %>% head(100) %>% as.vector()
cifar$classes <- ifelse(cifar$classes==1,1,0)
cifar %>% extract2("classes") %>% head(100) %>% as.vector()
```
