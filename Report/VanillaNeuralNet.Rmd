...
As shown, it took approximately 7 seconds to run this function. This is quite impressive, considering that there are over 320,000 parameters to be updated (see [Check Model Parameters](#CheckModelParameters) section).
...
```{r Run Model Training 1, eval=TRUE, echo=TRUE}
...
Notice here that the cost line begins at approximately `0.4`, and very quickly decreases to `0.4` after 20 Epochs. This can be seen as a success.
...
Here is an experimentation of trying some different parameters. Notice how the results also change.

```{r Run Model Training 2, eval=TRUE, echo=TRUE}
training_output <- let_TrainModel(
    x_train=trn_img, y_train=trn_cls,
    input_nodes=dim(trn_img)[2], hidden_nodes=c(100,75,50,30,20), output_nodes=1,
    initialisation_algorithm="xavier", initialisation_order="layers",
...
Since this article is about _showing_ the method, then we will not progress any further is more optimisation here.

```{r Confusion Matrix, eval=TRUE, echo=TRUE}
# Set Up
ConfusionMatrix <- Prediction %>% 
    mutate_at(c("truth","class"), ~ifelse(.==1,"car","not")) %>% 
    select(class,truth) %>% 
    table %>% 
...
```{r Plot Test Prediction, eval=TRUE, echo=TRUE, fig.width=5, fig.height=5, message=FALSE}
# Print
ConfusionMatrix %>% 
    plt_ConfusionMatrix()
```
