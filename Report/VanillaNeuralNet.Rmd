...
# Define function for retrieving attributes
GetAttributes <- function(object, name) {
...
PlotImage <- function(images, classes, index=1) {
    #' @title Plot Array
...
rm(classes, SliceArray, PlotArray)
```

```{r View Images, eval=TRUE, echo=TRUE}
for (i in 1:20) {
    PlotImage(cifa$images, cifa$classes, i)
}
```
...
InstantiateNetwork <- function(input=50, hidden=c(30,20,10), output=10) {
    ...
    for (layer in names) {
        ...
        if (layer=="input") {
            ...
            layer_index = str_sub(layer, -1) %>% IsWhole()
...
```{r Add Nodes, eval=TRUE, echo=TRUE}
...
names(network_model) %>% str_remove_all("hidden_") %>% as.numeric() %>% DescTools::IsWhole()
assert_that(all(DescTools::IsWhole(c(100,75,50,30,20))))
```
