...
for (layer in names(network_model)) {
    network_model[[layer]][["cost"]] <- ComputeCost(network_model[["output"]][["acti"]], trn_cls)
}
rm(layer)
```
...
```{r}
for (layer in names(network_model)) {
    network_model[[layer]][["back_cost"]] <- DifferentiateCost(network_model[["output"]][["acti"]], trn_cls)
    if (layer=="output") {
        network_model[[layer]][["back_acti"]] <- network_model[[layer]][["back_cost"]]
    }
}
rm(layer)
```
...
DifferentiateLinear <- function(back_linr_curr, acti_prev, wgts, bias) {
    ...
    diff_wgts <- 1/samp * (back_linr_curr %*% t(acti_prev))
    diff_bias <- 1/samp * colSums(back_linr_curr, dims=1)
...
sigmoid_backward <- function(diff_acti_curr, linr_curr) {
    ...
    diff_linr_curr <- diff_acti_curr * temp * (1-temp)
    return(diff_linr_curr)
}
```
...
BackwardProp <- function(network_model) {
    ...
    for (layr_indx in network_model %>% names %>% length %>% 1:. %>% rev) {
        ...
        # Apply back to model
        network_model[[layr_curr]][["back_acti"]] <- diff_acti_prev
